...
**Date:** 2026-02-23
**Status:** Draft
**Author:** Combined from tester-migration-v2.md and tester-migration-v3.md
...
The production system uses **three separate databases**, each with its own migration track:

1. **Project DB** — Application-specific tables and data
2. **Shared DB** — Shared library tables (common across all projects)
3. **AutoTester DB** — Test result storage for the AutoTester framework

The **Migration Tester** requires an additional **fourth database** for testing:

4. **DB-Under-Test (DUT)** — A dedicated test database where the goose migrator is exercised. Schema changes (e.g., `CREATE TABLE`) and the goose tracking table (`db_migrations`) live here. This database is separate from all production databases.

Additionally, the **goose migration system itself** requires its own database for tracking migration versions across all tracks. This is configured separately from the DUT.

**All databases must already exist before the tester runs.** The tester does not create or drop databases.

Configure the databases in the project's `mise.local.toml`:
...
PG_DB_NAME_MIGRATIONS = "<migrations_db>"

# DUT: Test database for MigrationTester (passed via MigrationTesterConfig)
# Not stored in mise.local.toml; configured programmatically in registry.go
```

### 3.3 `Prepare` Invariants
...
The system uses **four databases** in total. Configure them in `mise.local.toml`:
...
# Recommended naming: <project_db>_dut
```

### 11.2 MigrationTesterConfig
...
func registerAll(cfg *Config) {
    dutDB := openTestDB(cfg.DUTDSN) // Separate test database
...
|---|---|---|---|
| `DUTDB` | `*sql.DB` | — | Database connection for the test database |
...
### 11.5 Test Case Priority Distribution

...
<div class="grid gap-3" style="grid-template-columns: repeat({columns}, minmax(0, 1fr));">
	{#each icons as icon (icon.id)}
		...
		>
			...
			/>
			<span
				class="mt-1 text-xs text-center truncate w-full text-gray-700 dark:text-gray-300"
				title={icon.name}
			>
				{icon.name}
			</span>
...
{#if icons.length === 0}
	<div class="text-center py-8 text-gray-500 dark:text-gray-400">No icons found</div>
{/if}

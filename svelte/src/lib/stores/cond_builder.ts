...
class ConditionBuilder {
	...
	condEq(field_name: string, value: unknown, data_type: string = 'string'): this {
	...
	condGt(field_name: string, value: unknown, data_type: string = 'number'): this {
		this.conditions.push({
			type: 'atomic',
			field_name,
			opr: '>',
	...
	condGte(field_name: string, value: unknown, data_type: string = 'number'): this {
		this.conditions.push({
			type: 'atomic',
			field_name,
			opr: '>=',
	...
	condLt(field_name: string, value: unknown, data_type: string = 'number'): this {
		this.conditions.push({
			type: 'atomic',
			field_name,
			opr: '<',
	...
	condLte(field_name: string, value: unknown, data_type: string = 'number'): this {
		this.conditions.push({
			type: 'atomic',
			field_name,
			opr: '<=',
	...
	condNe(field_name: string, value: unknown, data_type: string = 'string'): this {
		this.conditions.push({
			type: 'atomic',
			field_name,
			opr: '<>',
	...
	condContains(field_name: string, value: unknown, data_type: string = 'string'): this {
		this.conditions.push({
			type: 'atomic',
			field_name,
			opr: 'contain',
	...
	condPrefix(field_name: string, value: unknown, data_type: string = 'string'): this {
		this.conditions.push({
			type: 'atomic',
			field_name,
			opr: 'prefix',
...
function tokenize(input: string): Token[] {
	...
	input = input.replace(/==/g, '=').replace(/&&/g, ' AND ').replace(/\|\|/g, ' OR ');
...
class ConditionParser {
	...
	private consume(type: TokenType, message: string): Token {
		...
		throw new Error(
			`${message} at position ${this.current}, got ${current.type}: ${current.value}`
		);
	}
}
...
export { cond_builder, ConditionConstructor, AndCondition, OrCondition };

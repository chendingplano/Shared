...
export type RecordInfo = { [key: string]: unknown };
...
export type StoreRecord = {
	InMemStoreCrtRecords: Writable<RecordInfo[]>;
...
export function InitInMemStore(store_name: string, init_view: string, limit_size: number = 200) {
	const componentStores = StoreMap.get(store_name);
	if (!componentStores) {
...
export function GetStoreByName(
	store_name: string,
	init_view: string = '',
	record_set_size: number = 200
): Writable<InMemStoreDef> {
	const existing = StoreMap.get(store_name);
	if (existing) {
		return existing;
	}

	...
	const results = await query_builder
		...
		.where(
			cond_builder
				.or()
				.addCond(
					cond_builder.and().condEq('status', 'active', 'string').condGt('age', 18, 'number')
				)
				.addCond(cond_builder.and().condEq('role', 'admin', 'string').condGt('level', 5, 'number'))
				.build()
		)
...
export async function example4_withSchema() {
	const results = await query_builder
		.select()
		.from('users')
		.where(cond_builder.filter().condEq('status', 'active', 'string').build())
...
export async function example5_leftJoin() {
	const results = await query_builder
		...
		.where(cond_builder.filter().condEq('users.status', 'active', 'string').build())
		.withSchema(UserSchema)
...
export async function example6_multipleJoins() {
	const results = await query_builder
		...
		.where(cond_builder.filter().condEq('posts.published', true, 'boolean').build())
...
export async function example7_pagination(page: number = 1, pageSize: number = 10) {
	...
	const results = await query_builder
		.select()
		.from('articles')
		.where(cond_builder.filter().condEq('status', 'published', 'string').build())

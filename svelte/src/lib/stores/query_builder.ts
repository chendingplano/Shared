...
//     .select()
//     .from('products')
//     .where(cond_builder.filter().condEq('category', 'electronics').build())
...
//     .from('users')
//     .where(cond_builder.filter().condEq('active', true).build())
//     .execute();
//
...
//     .select()
//     .from('articles')
//     .where(cond_builder.filter().condEq('status', 'published').build())
...
import type { CondDef, JoinDef, OrderbyDef, JimoResponse } from '$lib/types/CommonTypes';
...
class QueryConstructor {
	...
	/**
	 * Create a new query builder starting with the database name.
	 */
	database(dbName: string): QueryBuilder {
		const builder = new QueryBuilder();
		return builder.database(dbName);
	}
}
...
export { query_builder, QueryConstructor, QueryBuilder };

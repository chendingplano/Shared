...
import { CustomHttpStatus, type UserInfo } from '../types/CommonTypes';
import {
...
export const nowIso = (): IsoAutoDateString => new Date().toISOString() as IsoAutoDateString;

export type UsersResponse<Texpand = unknown> = Required<UserInfo> & AuthSystemFields<Texpand>;

// Deliberately excludes user_password and userToken: browser code must never
// request credential material. TODO: enforce server-side that these columns
// are not selectable via /shared_api/v1/jimo_req regardless of field_names.
...
export async function RetrieveUsers<T = UserInfo>(
	...
	const orderby_def = orderby_builder.start().orderby('user_name', 'string', true).build();
	...
	if (!response.status) {
		const error_msg =
			'Failed retrieving users (MRX_USR_035), error_msg:' +
			response.error_msg +
			', error code:' +
			response.error_code +
			', loc:' +
	...
	if (!Array.isArray(response.results)) {
		const error_msg = 'Expected array in response (loc:' + response.loc + ':' + loc + ')';
		if (alert_on_error) {
			alert(error_msg);
		} else {
			console.warn(error_msg);
...
export async function CreateUsersRecord(
	...
	_loc: string,
	_alert_on_error: boolean
): Promise<[UserInfo | null, number, string]> {
	if (!IsUserInfoRecord(record)) {
...
export async function UpdateUsersRecord(
	...
	const cond_def = cond_builder.filter().condEq('user_id', user_id, 'string').build();
	...
	if (resp.status) {
		if (resp.result_type === 'json') {
			const user_record = resp.results;
	...
	const error_msg = `failed updating users record, error:${resp.error_msg}, loc:${resp.loc}`;
...
export async function DeleteChecklistItemsRecord(
	...
	const cond_def = cond_builder.filter().condEq('user_id', id, 'string').build();

# Test Report: smoke-26032401

**Project:** shared  
**Package:** `github.com/chendingplano/shared/go/api/goose`  
**Generated:** 2026-03-24 20:14:54 UTC  
...
| 58 | NewWithDB with real PostgreSQL and empty migrations dir returns nil provider | `NewWithDB(pgDB, PgName, cfg{emptyDir}, logger)` | ✅ PASS | — | 0 |
| 59 | Up applies pending migrations against a real PostgreSQL database | `m.Up(ctx) — one pending migration` | ✅ PASS | — | 6 |
| 60 | HasPending returns false immediately after all migrations are applied | `m.HasPending(ctx)` | ✅ PASS | — | 0 |
| 61 | GetVersion returns the version number of the applied migration | `m.GetVersion(ctx)` | ✅ PASS | — | 0 |
| 62 | Status returns exactly one migration entry with Applied=true | `m.Status(ctx)` | ✅ PASS | — | 0 |
| 63 | Down rolls back the last applied migration | `m.Down(ctx)` | ✅ PASS | — | 1 |
| 64 | HasPending returns true after Down rolls back the migration | `m.HasPending(ctx) — after Down` | ✅ PASS | — | 0 |
| 65 | CreateAndApply creates migration file and applies it to PostgreSQL | `m.CreateAndApply(ctx, desc, createSQL, dropSQL)` | ✅ PASS | — | 2 |
| 66 | CreateAndApply leaves migration file on disk | `os.Stat(filepath.Join(dir, "20260324201454_create_pg_test_tbl_caa.sql"))` | ✅ PASS | — | 0 |
| 67 | CreateAndApply rebuilds provider so it is non-nil after application | `m.provider != nil` | ✅ PASS | — | 0 |
| 68 | Applied migration creates target table in PostgreSQL | `SELECT 1 FROM pg_test_tbl_caa LIMIT 1` | ✅ PASS | — | 0 |
| 69 | CreateAndApply with conflicting SQL surfaces PostgreSQL error at apply time | `m.CreateAndApply(ctx, desc, 'CREATE TABLE pg_test_tbl_caa', '') — no IF NOT EXISTS` | ✅ PASS | — | 0 |
| 70 | ListSources returns one entry per migration file in ascending order | `m.ListSources()` | ✅ PASS | — | 0 |
| 71 | UpTo applies only migrations up to and including the given version number | `m.UpTo(ctx, sources[0].Version)` | ✅ PASS | — | 2 |

---
*Generated by `goose_pg_test.go` — testname: `smoke-26032401`*

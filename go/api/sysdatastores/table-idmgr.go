...
func CreateIDMgrTable(
    ...
    if err != nil {
        error_msg := fmt.Errorf("failed creating table (SHD_IMG_045), err: %w, stmt:%s", err, stmt)
...
func UpsertActivityLogIDDef() error {
    ...
    if err != nil {
        error_msg := fmt.Sprintf("failed to insert activity_log_id record (SHD_IMG_045): %v, stmt:%s", err, stmt)
        log.Printf("***** Alarm %s", error_msg)
        return fmt.Errorf("%s", error_msg)
    }
